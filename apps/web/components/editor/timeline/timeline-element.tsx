...
import { MediaElement, TimelineElementProps } from "@/types/timeline";
...
export function TimelineElement({
  ...
  const handleToggleElementContext = (e: React.MouseEvent) => {
    ...
    if (hasAudio && (element.type === "video" || element.type === "audio")) {
      toggleSelectedMuted(
        isMultipleSelected && isCurrentElementSelected ? undefined : track.id,
        isMultipleSelected && isCurrentElementSelected ? undefined : element.id
      );
  ...
  const renderElementContent = () => {
    ...
    const mediaItem = mediaFiles.find(
      (file) => file.id === (element as MediaElement).mediaId
    );
    if (!mediaItem) {
      return (
  ...
  const isMuted =
    (element.type === "audio" || element.type === "video") && element.muted;
