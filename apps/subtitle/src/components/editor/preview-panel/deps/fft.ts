    ...
    for (let angleIndex = 1; angleIndex < eighth; angleIndex += 1) {
      ...
      do {
        for (let index = startInner; index < size; index += strideInner) {
          ...
          const tempCos = buffer[mirrorQuarter] * cos - buffer[quarterIndex] * sin;
          const tempSin = buffer[mirrorQuarter] * sin + buffer[quarterIndex] * cos;
          const mixCos = buffer[mirrorThreeQuarter] * cosFactor - buffer[threeQuarterIndex] * sinFactor;
          const mixSin = buffer[mirrorThreeQuarter] * sinFactor + buffer[threeQuarterIndex] * cosFactor;
