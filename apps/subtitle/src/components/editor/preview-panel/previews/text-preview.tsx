...
import { TextPreviewNode } from "../nodes/text-preview-node";
...
export function TextPreview(props: TextPreviewProps): null {
  const nodeRef = useRef<TextPreviewNode | null>(null);

  useEffect(() => {
    if (!props.parentLayer) {
      return undefined;
    }
    const node = new TextPreviewNode({
      id: props.id,
